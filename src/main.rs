/*
A domain has a set of allowed operations.
The operations can be parameterised.
An action is a parameterised operation. 
An action has a few methods, which allow us to:
    1. Determine the cost of the action
    2. Determine the resulting state
*/

// use pyo3_example::tile::v1;
...
fn main() {
    // let state = TileState::Standard(Tiles::from_tiles([0,1,2,3,4,5,6,7,8]));
    // let actions = state.actions();
    // let successors: Vec<TileState> = actions.iter().map(|a| a.apply()).collect();
    // println!("{actions:?}");
    // println!("{successors:?}");

    // let state = TileState::Cyclic(Tiles::from_tiles([0,1,2,3,4,5,6,7,8]));
    // let actions = state.actions();
    // let successors: Vec<TileState> = actions.iter().map(|a| a.apply()).collect();
    // println!("{actions:?}");
    // println!("{successors:?}")
